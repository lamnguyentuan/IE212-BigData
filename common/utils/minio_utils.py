...
class MinioClientWrapper:
    ...
    def __post_init__(self):
        ...
        if not self._client.bucket_exists(self.config.bucket):
            try:
                ...
                self.logger.info(f"[MinIO] Created bucket: {self.config.bucket}")
            except Exception as e:
                self.logger.error(f"[MinIO] Error creating bucket {self.config.bucket}: {e}")
    ...
    def check_silver_exists(self, video_id: str) -> bool:
        """
        Check if key artifacts exist in silver/{video_id}.
        Requires at least: video_embedding.npy, audio_embedding.npy, metadata_features.npz
        """
        # Minimal check: video_embedding.npy acts as flag
        obj_name = f"silver/{video_id}/video_embedding.npy"
        try:
            self._client.stat_object(self.config.bucket, obj_name)
            return True
        except Exception:
            return False

    def download_silver_files(self, video_id: str, data_root: Path) -> None:
        """
        Download 'silver/{video_id}/*' to 'data_root/silver/{video_id}/'
        Specifically needing: video_embedding.npy, audio_embedding.npy, metadata_features.npz
        """
        target_dir = data_root / "silver" / video_id
        ...
        artifacts = [
            ...
            "label.json" # Download label if exists
        ]

        for art in artifacts:
            obj_name = f"silver/{video_id}/{art}"
            local_path = target_dir / art
            try:
                self.download_object(obj_name, local_path)
            except Exception as e:
                # Expect label.json might fail
                if "label.json" not in art:
                     self.logger.warning(f"[MinIO] Failed to download {obj_name}: {e}")
                     raise e
